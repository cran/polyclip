    ...
    PolyFillType subjFillType, PolyFillType clipFillType)
{
  ...
  if (m_HasOpenPaths)
    THROWCLIPPER("Error: PolyTree struct is need for open path clipping.");
     ...
     const IntPoint &Pt, bool protect)
{
  ...
  if (e1->WindDelta == 0 || e2->WindDelta == 0)
  {
    ...
    if (e1stops)
      ...
      else THROWCLIPPER("Error intersecting polylines");
    if (e2stops) 
  ...
  for (;;)
  {
    ...
    while(e)
    {
        ...
        (dir == dRightToLeft && e->Curr.X >= horzLeft))
      {
	    ...
	    THROWCLIPPER("ProcessHorizontal error");
          return;
        }
    ...
    while( e->NextInSEL )
    {
      ...
      if(e->Curr.X > eNext->Curr.X)
      {
        if (!IntersectPoint(*e, *eNext, Pt, m_UseFullRange) && e->Curr.X > eNext->Curr.X +1)
          THROWCLIPPER("Intersection error");
        if (Pt.Y > botY)
        {
            Pt.Y = botY;
